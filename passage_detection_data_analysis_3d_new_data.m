...
for i = 1 : length(file_list)
    ...
    update_str = raw_str_data{7}(1, 1 : 6);
    if strcmp(update_str, 'Update') == 0
        continue
    end

    raw_data = zeros(50, 3);
    for row = 9:58
        str_data = raw_str_data{row, 1};
        split_str = split(str_data, ',');
        col_data = str2double(split_str)';
        raw_data(row - 8, :) = col_data;
    end
    ...
    for row = 62:111
        ...
        split_str = split(str_data, ',');
        col_data = str2double(split_str)';
        raw_data(row - 61, :) = col_data;
    end
    psg_detection_time_3d = raw_data(:, 1);
    psg_detection_time_2d = raw_data(:, 2);
    compound_cell_detection_time = raw_data(:, 3);
    cell_detection_time_2d = raw_data(:, 4);      
    ...
    %                  8-average 3d passage detection time, 9-average 2d passage detection time, 
    %                  10-average compound cell detection time,
    %                  11-average cell detection time]
    ...
    res(res_row, 10) = sum(compound_cell_detection_time) / trial_num;
    res(res_row, 11) = sum(cell_detection_time_2d) / trial_num;
end
...
set(gca, 'FontSize', 18)
ylabel('Passage/Cell number', 'FontName', 'Arial', 'FontSize', 20);
xlabel('Obstacle number (3D)', 'FontName', 'Arial', 'FontSize', 20);
leg_1 = legend({'3D passages', 'Passages on the base', 'Cells on the base'},'FontName', 'Arial', 'FontSize', 20, 'Location', 'northwest');
...
hold on
plot(res(:, 1), res(:, 10), 'LineWidth', 2, 'Marker','+', 'MarkerSize', 10)

set(gca, 'FontSize', 18)
ylabel('Detection time (ms)', 'FontName', 'Arial', 'FontSize', 20);
xlabel('Obstacle number (3D)', 'FontName', 'Arial', 'FontSize', 20);
leg_1 = legend({'3D passage detection', 'Planar passage detection', 'Compound cell detection'},'FontName', 'Arial', 'FontSize', 20, 'Location', 'northwest');
ax = gca;
ax.XGrid = 'off';
ax.YGrid = 'on';
xlim([0, 205])
