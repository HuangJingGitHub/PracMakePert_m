classdef teleOp_test_trajectory < handle
  properties(Access = public)
    ...
    methods(Access = public)
        ...
        function obj = teleOp_test_trajectory(mtm_q_initial,psm_q_initial) %publisher,jointStateMsg)  % Constructor
            ...
            obj.psm_x_dsr_0902  = obj.psm_x_cur;
        ...
        function  [psm_q, tracking_err] = run(obj, mtm_q)
            ...
            obj.psm_x_dsr   = obj.psm_x_dsr_0902 + [0 0 0 0.005*cos(0.01*pi*obj.counter);
                                                    0 0 0 0.005*sin(0.01*pi*obj.counter);
                                                    0 0 0 0;
                                                    0 0 0 0];
            obj.test_0902(:, obj.counter) = obj.psm_x_dsr(1:3, 4);
            %%% ***Test on position tracking*** %%%
            %obj.psm_x_dsr(1:3, 4) = zeros(3, 1); 
            
            %%% ***Test on pure rotation*** %%%
            alpha = 2 * pi * 1e-6 * obj.counter;
            rotationAxis = obj.psm_x_dsr(1:3, 3);
            tipPosition = obj.psm_x_dsr(1:3, 4);
            rotationMatrix = angleAxisMatrix(rotationAxis, alpha, tipPosition);
            %obj.psm_x_dsr  = rotationMatrix * obj.psm_x_dsr; 
            %%% ***Test on position tracking*** %%%           
