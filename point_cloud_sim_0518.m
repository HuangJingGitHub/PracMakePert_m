...
for row = 1 : size(xy_z, 1)
    for col = 1 : size(xy_z, 2)
        temp_x = xy_x(row, col);
        xy_z_deformed(row, col) = xy_z(row, col) + 20 - 20 * sin(omega * temp_x);
        temp_y = xy_y(row, col);
...
for row = 1 : size(xz_z, 1)
    for col = 1 : size(xz_z, 2)
        temp_x = xz_x(row, col);
        xz_z_deformed(row, col) = xz_z(row, col) + 20 - 20 * sin(omega * temp_x);
        temp_y = xz_y(row, col);
...
for i = 1 : boundary_pts_num
    ...
    s_g_dis_weighted_quad = s_g_dis_weighted_quad + boundary_set_deformed(:, i) * dis_weight_quad;
end
...
for i = 1 : boundary_pts_num
    ...
    s_g_dis_weighted_quad_to_boundary(1, i) = norm(s_g_dis_weighted_quad - boundary_set_deformed(:, i));
end
...
% save('flexion_s_g_max_dis.mat', 's_g_proj_log');
% save('flexion_s_g_dis_weight.mat', 's_g_dis_weighted_proj_log');
% save('flexion_s_g_dis_weight_quad.mat', 's_g_dis_weighted_quad_proj_log')

%% plot
load('flexion_xy_z.mat')
load('flexion_xz_z.mat')
load('flexion_s_g_max_dis.mat')
load('flexion_s_g_dis_weight.mat')
load('flexion_s_g_dis_weight_quad.mat')
...
for row = 1 : size(xz_x, 1)
    ...
    hold on;
    scatter3(xz_x(row, :), xz_y(row, :), xz_z_deformed_log(row, :), ...
             'MarkerFaceColor', 'b', 'MarkerEdgeColor', 'k', 'MarkerFaceAlpha', 0.8);    
end
