...
%%
err_reduction = zeros(6, 2);
for i = 1 : 6
    err_reduction(i, 1) = (average_err(i, 1) - average_err(i, 2)) / average_err(i, 1);
end

for i = 1 : size(fabric_2pt_dm, 2)
    err_reduction(1, 2) = max(err_reduction(1, 2), (fabric_2pt_dm(1, i) - fabric_2pt_mm(1, i)) / fabric_2pt_dm(1, i));
end
for i = 1 : size(fabric_3pt_dm, 2)
    err_reduction(2, 2) = max(err_reduction(2, 2), (fabric_3pt_dm(1, i) - fabric_3pt_mm(1, i)) / fabric_3pt_dm(1, i));
end
for i = 1 : size(fabric_4pt_dm, 2)
    err_reduction(3, 2) = max(err_reduction(3, 2), (fabric_4pt_dm(1, i) - fabric_4pt_mm(1, i)) / fabric_4pt_dm(1, i));
end
for i = 1 : size(sponge_1pt_dm, 2)
    err_reduction(4, 2) = max(err_reduction(4, 2), (sponge_1pt_dm(1, i) - sponge_1pt_mm(1, i)) / sponge_1pt_dm(1, i));
end
for i = 1 : size(sponge_2pt_dm, 2)
    err_reduction(5, 2) = max(err_reduction(5, 2), (sponge_2pt_dm(1, i) - sponge_2pt_mm(1, i)) / sponge_2pt_dm(1, i));
end
for i = 1 : size(sponge_3pt_dm, 2)
    err_reduction(6, 2) = max(err_reduction(6, 2), (sponge_3pt_dm(1, i) - sponge_3pt_mm(1, i)) / sponge_3pt_dm(1, i));
end
